 ...
 * 通过RecyclerView展示线性天气的 item用的view (两条线和圆点)
 */
...
public class WeatherLineView extends View {
     ...
     * A线的颜色,B线的颜色
     */
    private int mLineColorA = Color.YELLOW;
    private int mLineColorB = Color.BLUE;
    ...
    private void initPaint() {
        ...
        mLinePaintA.setStrokeWidth(mWeaLineWidth);
        mLinePaintB.setStrokeWidth(mWeaLineWidth);
        mLinePaintA.setColor(mLineColorA);
        mLinePaintB.setColor(mLineColorB);
    }
    /**
     * 获取自定义属性并赋初始值
     */
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mWeaTextColor = a.getColor(R.styleable.WeatherLineView_weaTextColor, Color.parseColor("#333333"));
        mLineColorA = a.getColor(R.styleable.WeatherLineView_weaLineAColor,Color.parseColor("#fee55b"));
        mLineColorB = a.getColor(R.styleable.WeatherLineView_weaLineBColor,Color.parseColor("#2173c0"));
        mDotColor = a.getColor(R.styleable.WeatherLineView_weaDotBColor,Color.parseColor("#666666"));
        ...
        isTwo = a.getBoolean(R.styleable.WeatherLineView_weaLineIsTwo,true);
        a.recycle();
    }
    @Override
        ...
        if (mode == MeasureSpec.EXACTLY) {
            ...
            if (type == 0) {
                ...
                result = dp2px(getContext(), defaultMinHeight) + textHeight +
                        // 需要加上两个文字和圆点的间距
                        getPaddingTop() + getPaddingBottom() + mTextDotDistance;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLowTemperData[3] == 0) {
            // 最低温度左边
            int calowLeft = baseHeight - cacHeight(mLowTemperData[0]);
            canvas.drawLine(0, calowLeft, getWidth() / 2, calowMiddle, mLinePaintB);
         //   Log.i("calow","left x(0,"+calowLeft+")\t"+"left y("+getWidth() / 2+","+ calowMiddle+")\t"+"温度："+mLowTemperData[1]);
        }

        if (mLowTemperData[4] == 0) {
            // 最低温度右边
            int calowRight = baseHeight - cacHeight(mLowTemperData[2]);
            canvas.drawLine(getWidth() / 2, calowMiddle, getWidth(), calowRight, mLinePaintB);
         //   Log.i("calow","right x("+getWidth() / 2 +","+calowMiddle+")\t"+"right y("+getWidth()+","+ calowRight+")	"+"温度："+mLowTemperData[1]);
        }
       ...
       if(isTwo){
           ...
           if (mHighTemperData[3] == 0) {
               // 最高温度左边
               int calHighLeft = baseHeight - cacHeight(mHighTemperData[0]);
               canvas.drawLine(0, calHighLeft, getWidth() / 2, calHighMiddle, mLinePaintA);
           }

           if (mHighTemperData[4] == 0) {
               // 最高温度右边
               int calHighRight = baseHeight - cacHeight(mHighTemperData[2]);
               canvas.drawLine(getWidth() / 2, calHighMiddle, getWidth(), calHighRight, mLinePaintA);
           }
       }
    }
