...
import com.risenb.ykj.utlis.DisplayUtil;
...
public class Today24HourView extends View{
    ...
    private int ITEM_WIDTH = 60; //每个Item的宽度
    private int MARGIN_LEFT_ITEM = 50; //左边预留宽度
    private int MARGIN_RIGHT_ITEM = 50; //右边预留宽度

    private int windyBoxAlpha = ITEM_WIDTH/2;
    private int windyBoxMaxHeight = ITEM_WIDTH/2;
    private int windyBoxMinHeight = ITEM_WIDTH/10;
    private int windyBoxSubHeight = windyBoxMaxHeight - windyBoxMinHeight;
    private int bottomTextHeight = 30;

    private int mHeight=150, mWidth;
   ...
   // private int textSize = 12;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Today24HourView,
                defStyleAttr, 0);
        pointColor = a.getColor(R.styleable.Today24HourView_lineAndTextColor, Color.parseColor("#ffffff"));
        lineColor = a.getColor(R.styleable.Today24HourView_todayHourLineColor,Color.parseColor("#fee55b"));
        ITEM_WIDTH = (int) a.getDimension(R.styleable.Today24HourView_itemWidth,
                DisplayUtil.dip2px(context, ITEM_WIDTH));
        mHeight = (int) a.getDimension(R.styleable.Today24HourView_viewHeight,
                DisplayUtil.dip2px(context, mHeight));
        bottomTextHeight = (int) a.getDimension(R.styleable.Today24HourView_bottomTextHeight,
                DisplayUtil.dip2px(context, bottomTextHeight));
        MARGIN_LEFT_ITEM = (int) a.getDimension(R.styleable.Today24HourView_marginLeftItem,
                DisplayUtil.dip2px(context, MARGIN_LEFT_ITEM));
        MARGIN_RIGHT_ITEM = (int) a.getDimension(R.styleable.Today24HourView_marginRightItem,
                DisplayUtil.dip2px(context, MARGIN_RIGHT_ITEM));
//        textSize = (int) a.getDimension(R.styleable.Today24HourView_textSize,
//                DisplayUtil.dip2px(context, textSize));
        dotSize = (int) a.getDimension(R.styleable.Today24HourView_todayHourDotSize,
                DisplayUtil.dip2px(context, dotSize));
        unitText = a.getString(R.styleable.Today24HourView_todayHourUnitText);
        a.recycle();
    ...
    private void initPaint() {
        ...
        dashLinePaint.setColor(pointColor);
        ...
        windyBoxPaint.setColor(pointColor);
    ...
    public void setHourItems(List<HourItem> list){
        ...
        if(listItems!=null&&listItems.size()>0){
            List<HourItem> tt = sort("Temp");
            minTemp = tt.get(0).getTemperature();
            maxTemp = tt.get(tt.size()-1).getTemperature();
            List<HourItem> ww = sort("Windy");
            maxWindy = ww.get(ww.size()-1).getWindy();
            minWindy = ww.get(0).getWindy();
            for (int i = 0; i <listItems.size() ; i++) {
                int left =MARGIN_LEFT_ITEM  +  i * ITEM_WIDTH;
                int right = left + ITEM_WIDTH - 1;
                int top = (int)(mHeight -bottomTextHeight +
                        (maxWindy - listItems.get(i).getWindy())*1.0/(maxWindy - minWindy)*windyBoxSubHeight
                        - windyBoxMaxHeight);
                int bottom =  mHeight - bottomTextHeight;
                Rect rect = new Rect(left, top, right, bottom);
                Point point = calculateTempPoint(left, right, listItems.get(i).getTemperature());
                listItems.get(i).setWindyBoxRect(rect);
                listItems.get(i).setTempPoint(point);
            }
            invalidate();
        }
    }
...
//        中间温度的虚线
    ...
    private void onDrawTemp(Canvas canvas, int i) {
        ...
        canvas.drawCircle(point.x, point.y, dotSize, pointPaint);
    ...
    public void drawLeftTempText(Canvas canvas, int offset){
        ...
        canvas.drawText(unitText, DisplayUtil.dip2px(getContext(), 6) + offset, mHeight - bottomTextHeight-5, textPaint);
    }
...
//        Log.d(TAG, "maxScrollOffset = " + maxScrollOffset + "  scrollOffset = " + scrollOffset);
    ...
    private List<HourItem> sort(String agr) {
        List<HourItem> list = new ArrayList<>();
        list.addAll(listItems);
        if(agr.equals("Temp")){
            Collections.sort(list, new Comparator<HourItem>() {
                @Override
                public int compare(HourItem lhs,
                                   HourItem rhs) {
                    // 排序找到温度最低的，按照最低温度升序排列
                    return lhs.getTemperature() - rhs.getTemperature();
                }
            });
        }else if(agr.equals("Windy")){
            Collections.sort(list, new Comparator<HourItem>() {
                @Override
                public int compare(HourItem lhs,
                                   HourItem rhs) {
                    // 排序找到风力最低的，按照最低风力升序排列
                    return lhs.getWindy() - rhs.getWindy();
                }
            });
        }
        return list;
